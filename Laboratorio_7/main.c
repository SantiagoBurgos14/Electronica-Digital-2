...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    fres = f_mount(&fs, "", 1);
      if (fres != FR_OK) {
      	sprintf(buffer, "Error al montar la SD: %d\n", fres);
  ...
  /* USER CODE BEGIN WHILE */
	while (1) {
		...
		char opcion = recibir_uart();		// Se lee la opcion elegida
		transmit_uart("\n");

		switch (opcion) {
		case '1':
			SD_Imprimir("ascii-art.txt.txt");		// Muestra la imagen asci-art
			break;
		case '2':
			SD_Imprimir("muertito.txt.txt");		// Muestra la imagen muertito
			break;
		case '3':
			SD_Imprimir("pelea.txt.txt");			// Muestra la imagen pelea
			break;
		case '4':
			transmit_uart("Saliendo...\n");
			SD_Desmontar();							// Desmonta la SD
			while (1)
				;  // Detiene ejecución
			break;
		default:
			transmit_uart("Opcion invalida. Intente de nuevo.\n");
			break;
		}
		HAL_Delay(1000);  // Pausa entre iteraciones del menú
	}
    /* USER CODE END WHILE */
...
/* USER CODE BEGIN 4 */
void menu(void) {										// Funcion de Menu

transmit_uart("\n----- MENU SD -----\n");
transmit_uart("1. Ver imagen ascii-art.txt\n");
transmit_uart("2. Ver imagen muertito.txt\n");
transmit_uart("3. Ver imagen pelea.txt\n");
...
//transmit_uart("Intentando abrir: ");
//transmit_uart((char*) filename);
//transmit_uart("\n");
